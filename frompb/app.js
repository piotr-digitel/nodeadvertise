...
//edit adv - id from param, data to change from body
    app.patch('/adv', async (req, res, next) => {
        ...
        if(validationRegex.test(id)){                                               //is id format ok?
            try { 
                const adv = await getAdv(id);                                       //record(id) exist?
                if(adv){
                    if(adv.author===res.userLogin){                                 //logged user === author?
                        ...
                        res.statusCode = 401;
                        res.send('The request requires author authentication.');      //logged user has no permition to do that 
                    };
                }else{
                    res.statusCode = status.NOT_FOUND;
                    res.send('Record does not exist.');   
